...
*(Heads-up added 2026-06-12: the `# RDPI Workflow Rules` section was removed from CLAUDE.md and `.claude/templates/playbook-sections.md` — RDPI is no longer enforced from CLAUDE.md, and a broader product refactor is planned. AC 6's "under RDPI workflow rules" placement no longer exists; evaluate during this task's RDPI where (or whether) the cross-context triage rule lands in CLAUDE.md.)*
...
*(Heads-up added 2026-06-12: the `# RDPI Workflow Rules` section — including the Phase 1/2 descriptions ACs 4–6 edit — was removed from CLAUDE.md and `.claude/templates/playbook-sections.md`; a broader product refactor is planned. Evaluate during this task's RDPI whether those ACs still apply and where any CLAUDE.md-level phase documentation should live.)*
...
*(Heads-up added 2026-06-12: the `# RDPI Workflow Rules` section was removed from CLAUDE.md and `.claude/templates/playbook-sections.md`; a broader product refactor is planned. AC 1's placement ("near the existing triage rule", which itself targeted the removed section) needs re-evaluation during this task's RDPI.)*
...
9. Lifecycle references to the old `tasks/checkpoint.md` are reconciled to the new location/behavior: `.claude/skills/finish/SKILL.md:39` (cleanup — no longer needs `git rm` since the file is gitignored), `.claude/skills/playbook-audit/SKILL.md:65` (artifact list), `.claude/skills/playbook-update/SKILL.md:267` (maintainer-artifact check), `.claude/skills/playbook-setup/SKILL.md:151`, and the mirrored `.claude/templates/playbook-sections.md:168` (the CLAUDE.md session-start text is appended from this template on existing-project setup, so a CLAUDE.md-only edit is lost on fresh installs).
   *(Note added 2026-06-11: the "Session-Start Validation" section — including the active-checkpoint check formerly at `CLAUDE.md:213` — was removed from CLAUDE.md entirely during a cleanup pass. This task must decide where (and whether) a checkpoint-pickup notice belongs in CLAUDE.md — possibly nowhere, if the task-18 SessionStart hook makes it redundant — and verify the placement with the developer. Steer: keep CLAUDE.md lean; do not re-grow a validation section. The mirrored session-start text in `.claude/templates/playbook-sections.md:168` still carries the old wording and must be reconciled to whatever is decided.)*
...
- Lifecycle refs to reconcile: CLAUDE.md (checkpoint-pickup notice placement — see AC 9 note), `.claude/skills/finish/SKILL.md:39`, `.claude/skills/playbook-audit/SKILL.md:65`, `.claude/skills/playbook-update/SKILL.md:267`, `.claude/skills/playbook-setup/SKILL.md:151`, `.claude/templates/playbook-sections.md:168`.
