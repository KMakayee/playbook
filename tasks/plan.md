...
- [x] **Smoke test (operator-monitored):** after Phase 1 commits, the operator runs one representative backgrounded `codex exec` invocation (e.g., trigger `/implement` Step 6 or `/issue-implement` Step 6 in a real workflow) and confirms it completes and produces the expected output artifact. Per `tasks/issues.md:119`: if a run still hangs after the fix, the cause is something other than fd-0 reads (`/dev/tty`, permission prompts, model timeouts) — not evidence the redirect failed. **Satisfied 2026-05-02:** the `/implement` Step 6 Codex code review (a representative backgrounded `codex exec` invocation against the post-Phase-1 working tree) completed with exit 0 and produced `tasks/codex-code-review.tmp` (14 lines).
