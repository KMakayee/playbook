...
- Verify `tasks/plan-issue-$ARGUMENTS.md` exists. If not, stop — run `/issue-plan $ARGUMENTS` first.
...
- Read `tasks/plan-issue-$ARGUMENTS.md` FULLY — use the Read tool WITHOUT limit/offset parameters.
- Understand the recommended approach reference, phase structure, and success criteria.
- Read `tasks/research-issue-$ARGUMENTS.md` and the issue body in `tasks/issues.md` if you need additional context on specific files or patterns.
...
### 3.5. Update issue status to In Progress

In `tasks/issues.md`, change issue #$ARGUMENTS status to `In Progress`.
...
c. **Handle mismatches:**
   ...
   - **Structural** (module reorganized, interface changed, file deleted): STOP. Run a top-level Codex call to re-research the mismatch (replace `{phase}` with the current phase number, e.g. `tasks/codex-debug-issue-$ARGUMENTS-3.tmp`):
     ```bash
     codex -c model_reasoning_effort=xhigh exec \
       --sandbox read-only \
       -o tasks/codex-debug-issue-$ARGUMENTS-{phase}.tmp \
       "Re-research a structural mismatch encountered while implementing tasks/plan-issue-$ARGUMENTS.md.
     ...
     Inspect tasks/plan-issue-$ARGUMENTS.md (the original premise) and tasks/research-issue-$ARGUMENTS.md (the original sweep), then sweep the current code at the cited paths. Return: actual structure and locations, what the plan assumed, and a delta description Claude can use to update the plan. Be specific with file paths and line numbers.
     ...
     ```
     Verify the output before reading: `bash .claude/scripts/codex-output-check.sh tasks/codex-debug-issue-$ARGUMENTS-{phase}.tmp 5`. If the check fails, stop and tell the developer.

     Use a 10-minute timeout (600000ms). Read the output, adapt the plan, and continue. (Sub-agents are no longer used here — Codex sweeps faster on read-only structural questions, and the recursion guard at `CLAUDE.md` foreclosed Codex-from-inside-sub-agents anyway.)
   - **Plan premise invalidated** (the mechanism the plan specified doesn't actually work as described): document the deviation in `tasks/plan-issue-$ARGUMENTS.md`, adapt while preserving the step's intent, and continue. If the deviation affects the recommended approach — not just the mechanism — STOP and revisit the plan.
...
- Were any files changed that the plan didn't call for? (Note: \`tasks/plan-issue-$ARGUMENTS.md\` may be updated during implementation — checkmarks, deviation notes — do not flag this as scope drift.)
...
PART 3 — Acceptance criteria coverage:
- For each acceptance criterion in issue #$ARGUMENTS in tasks/issues.md, mark it Covered (with the file:line where it's implemented) or Missing (flag as a CORRECTION, since acceptance criteria are factual inputs).
...
**Check:** After the backgrounded Codex process completes, verify the output: `bash .claude/scripts/codex-output-check.sh tasks/codex-issue-code-review-$ARGUMENTS.tmp 10`. If the check fails, stop and tell the developer.

### 7. Triage findings

Read `tasks/codex-issue-code-review-$ARGUMENTS.tmp` FULLY.
...
- **Fix:** Bugs, missing error handling, missing acceptance-criteria coverage, genuine simplification wins, pattern violations — anything where the fix is clear and scoped.
...
You are running non-interactively — do not ask questions." --dangerously-skip-permissions > tasks/logs/code-review-fixes-issue-$ARGUMENTS-$TIMESTAMP.log 2>&1
```
...
- Any `tasks/codex-debug-issue-$ARGUMENTS-*.tmp` files (one per structural-mismatch Codex call, if any fired during phases)

### 10.5. Update issue status to Implemented

In `tasks/issues.md`, change issue #$ARGUMENTS status to `Implemented`.
...
Suggest next step: "Run `/issue-update $ARGUMENTS` next."
