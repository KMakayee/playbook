...
1. **Check prerequisites:**
   - Verify `tasks/plan-issue-$ARGUMENTS.md` exists. If not, stop and tell the developer to run `/issue-plan $ARGUMENTS` first.
...
3. **Check for resume:**
   ...
   - If found, ask the developer: "Found existing progress in plan-issue-$ARGUMENTS.md. Resume from where it left off, or start fresh?"
...
5. **Execute phase-by-phase:**
   ...
   d. **Run automated verification** — execute the success criteria listed in the plan for this phase. Fix any failures before proceeding.
...
7. **Commit artifacts.** Include `tasks/research-issue-$ARGUMENTS.md` and `tasks/plan-issue-$ARGUMENTS.md` in the final commit — do NOT remove them.
   - Do NOT remove `tasks/deferred.md`

8. **Update issue status.** In `tasks/issues.md`, change issue #$ARGUMENTS status to `Implemented`.

9. **Suggest next step.** Tell the developer: "Run `/issue-code-review-codex $ARGUMENTS` to review the implementation."
...
- Progress lives in `tasks/plan-issue-$ARGUMENTS.md` checkboxes — no separate todo file needed. The plan IS the progress tracker.
...
- **Compaction**: For multi-phase plans, compact between phases — drop file contents from completed phases and verbose test output, keep plan-issue-$ARGUMENTS.md location, current phase, and what's next.
