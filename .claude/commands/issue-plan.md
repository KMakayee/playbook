...
3. **Create structure outline:**
   - Based on the research findings and recommended approach, outline the high-level implementation phases.
...
5. **Write detailed plan per phase:**
   ...
   - Describe what changes are needed and why (traceable to the recommended approach or acceptance criteria).
...
6. **Write the plan artifact** to `tasks/plan-issue-$ARGUMENTS.md`:
   ...
   - The plan must include these elements (in whatever structure makes sense):
     ...
     - **Recommended approach** — which approach was chosen and why (from the research artifact's Recommended Approach section)
     ...
     - **Artifact reference** — pointer to `tasks/research-issue-$ARGUMENTS.md`

7. **Verify the draft plan:**
   - Cross-check against ALL acceptance criteria in the issue — every criterion should be addressed by at least one plan step.
   - Cross-check against the research artifact's open questions and verification notes. Every item flagged there should either appear in the plan or be noted as intentionally deferred with reasoning.
...
8. **Update issue status.** In `tasks/issues.md`, change issue #$ARGUMENTS status to `In Planning`.

9. **Present summary to the developer:**
    ...
    - Do NOT implement until the plan is approved. If rejected or revised, update `tasks/plan-issue-$ARGUMENTS.md` before proceeding.
...
- The recommended approach in the research is already chosen. Do not revisit or re-evaluate options — plan the execution.
...
- Every changed file needs a reason traceable to the recommended approach or acceptance criteria. If you can't explain why a file is changing, remove it from the plan.
...
- If `tasks/plan-issue-$ARGUMENTS.md` already exists, warn the developer and ask whether to overwrite or abort.
