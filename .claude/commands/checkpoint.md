...
4. **`base_head` check** — compare `base_head:` against current HEAD ancestry using `git merge-base --is-ancestor <base_head> HEAD`. If `base_head` is not an ancestor of current HEAD, warn that the diff's base is no longer reachable and recommend the developer inspect the embedded diff before continuing. Otherwise, if `base_head` differs from current short HEAD but is reachable, note it in the resume report (informational).
