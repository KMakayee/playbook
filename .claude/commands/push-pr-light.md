...
2. **Staleness gate** — Verify the current branch is up to date with its base before pushing.
   ...
   - **If non-zero:** stop. Tell the developer: "Branch is N commits behind `<base>`. Run `/catchup`, then re-run `/push-pr-light`." Do not proceed. (Cannot programmatically invoke `/catchup` — see `/checkpoint`'s `/compact` recommendation pattern.)
   - **If zero:** continue.
