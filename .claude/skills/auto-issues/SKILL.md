---
name: auto-issues
description: Run the full issue pipeline for one issue end-to-end, unattended, inside a dedicated worktree.
...
```bash
claude -p "Read .claude/skills/issue-research/SKILL.md and follow its instructions exactly for issue #N — follow the Markdown body and ignore the leading YAML frontmatter. You are running non-interactively — do not ask questions, make reasonable choices and proceed." --permission-mode auto </dev/null > tasks/logs/auto-issue-N-1-research-$TIMESTAMP.log 2>&1
```
...
```bash
claude -p "Read .claude/skills/issue-plan/SKILL.md and follow its instructions exactly for issue #N — follow the Markdown body and ignore the leading YAML frontmatter. You are running non-interactively — do not ask questions or wait for approval. Make reasonable choices and proceed." --permission-mode auto </dev/null > tasks/logs/auto-issue-N-2-plan-$TIMESTAMP.log 2>&1
```
...
```bash
claude -p "Read .claude/skills/issue-implement/SKILL.md and follow its instructions exactly for issue #N — follow the Markdown body and ignore the leading YAML frontmatter. You are running non-interactively — the plan is approved, proceed with implementation. Do not ask questions. If you hit a structural mismatch, follow .claude/skills/issue-implement/SKILL.md's structural-mismatch handling exactly (Step 4c)." --permission-mode auto </dev/null > tasks/logs/auto-issue-N-3-implement-$TIMESTAMP.log 2>&1
```
...
```bash
claude -p "Read .claude/skills/issue-update/SKILL.md and follow its instructions exactly for issue #N — follow the Markdown body and ignore the leading YAML frontmatter. You are running non-interactively — do not ask questions." --permission-mode auto </dev/null > tasks/logs/auto-issue-N-4-update-$TIMESTAMP.log 2>&1
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/pipeline-eval.sh N $TIMESTAMP
```
