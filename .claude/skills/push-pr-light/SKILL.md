---
name: push-pr-light
description: Push the current branch and open or update a PR (lightweight, no verification gate).
...
4. Check whether an open PR already exists for this branch:
   ...
   - **If no open PR and `gh` is installed:**
     ...
     3. Run `gh pr create --title "<title>" --fill`. `--title` overrides `--fill`'s auto-derivation; `--fill` still supplies the body from commits. If the user provided a base branch argument (e.g., `/push-pr-light main`), add `--base <branch>`.
...
6. **Conditional merge** — Evaluate the light review result:
   ...
   - **Merge error handling** — If `gh pr merge` fails:
     ...
     3. If the fresh PR also fails: stop, log the error to `tasks/errors.md` (use the format in `.claude/templates/error-report.md`), and report the issue to the developer.
...
9. **Reflect** — Scan the reflection prompt in `.claude/templates/error-report.md`. If anything from this session is worth logging, append a learning entry to `tasks/errors.md`.
