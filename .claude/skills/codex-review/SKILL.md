---
name: codex-review
description: A one-shot Codex second-opinion pass over a file, diff, artifact, or freeform description.
argument-hint: '[file | diff | artifact | "description"]'
when_to_use: 'Use when asked to "have codex review", "get a codex opinion", or "codex review this".'
---
...
**Target resolution.** This skill has two entry modes:

- **Explicit target** — `$ARGUMENTS` is non-empty (e.g., `/codex-review path/to/file.ts`, `/codex-review tasks/design-decision.md`, or `/codex-review "the retry-budget assumption in the ingest worker"`). Review that target unchanged.
- **No explicit target** — `$ARGUMENTS` is empty or blank (auto-fired mid-conversation, or a bare `/codex-review`). Infer the target from conversational context — the work just completed, or whatever the chat instruction tagged. If the target is genuinely ambiguous, ask one clarifying question (e.g., "review all of this, or just X?") rather than guessing or stopping.

Wherever the steps below reference `$ARGUMENTS` as the target, use the resolved target — the explicit `$ARGUMENTS` value, or the inferred description when no argument was supplied.
...
Write the composed prompt body to `tasks/codex-review-prompt.tmp` first — `$ARGUMENTS` may contain quotes, backticks, or newlines, and writing to a tmp file then reading via `"$(cat ...)"` avoids shell-quoting hazards. This mirrors the convention in `.claude/skills/research-codebase/SKILL.md`.

The prompt body interpolated into `tasks/codex-review-prompt.tmp` must contain (use the **resolved target** from Target resolution above — when an explicit `$ARGUMENTS` was supplied, interpolate it literally and do NOT preprocess, classify, or rewrite it; when the target was inferred, write the inferred target description in its place):

```
Target to review: <resolved target>
...
This matches the cleanup-then-present convention in `.claude/skills/research-codebase/SKILL.md`, `.claude/skills/design/SKILL.md`, and `.claude/skills/create-plan/SKILL.md`. Cleanup-before-present is load-bearing here because Step 6 ends in an interactive prompt — if cleanup were after, an unanswered offer would leave temp files behind. Triage in Step 6 operates on Codex's output already loaded into Claude's context (from Step 4) plus on-demand reads of the cited source files; it does not need the temp file. This command does NOT add itself to `/finish`'s cleanup list — its temp files are deleted in this step.
